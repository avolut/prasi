...
const walk = async (
  ...
  if (val.cmeta && val.cmeta.item.type === "item") {
    const child_ids = val.cmeta.comp?.child_ids;
    if (child_ids) {
      item = fillID(item, (e) => {
        if (child_ids[e.id]) {
          e.id = child_ids[e.id];
  ...
  if (item) {
    ...
    if (item.type === "item" && item.component?.id) {
      ...
      if (doc) {
        ...
        if (mcomp) {
          const child_ids: any = {};
          const icomps = await instantiateComp(item, mcomp, child_ids);
